# A torch-geometric model trainer to properly incorporate Thu-ChenXiang electrolyte database.
# Part of this code are borrowed from dive-into-graph project,
# see https://github.com/divelab/DIG/blob/dig-stable/dig/threedgraph/method/run.py
...
class pyG_trainer():
    ...
    def run(self, device, train_dataset, valid_dataset, test_dataset, model, loss_func, evaluation, epochs=500,
        ...
        if sampler==None:
            ...
            valid_loader = DataLoader(valid_dataset, vt_batch_size, shuffle=False, num_workers=num_workers)
            test_loader = DataLoader(test_dataset, vt_batch_size, shuffle=False, num_workers=num_workers)
        else:
            train_loader = DataLoader(train_dataset, batch_size, shuffle=True, num_workers=num_workers, sampler=sampler)
            valid_loader = DataLoader(valid_dataset, vt_batch_size, shuffle=False, num_workers=num_workers, sampler=sampler)
            test_loader = DataLoader(test_dataset, vt_batch_size, shuffle=False, num_workers=num_workers, sampler=sampler)
    ...
    def runCLR(self, device, train_dataset, valid_dataset, model, loss_func, evaluation, epochs=500,
        ...
        if sampler==None:
            train_loader = DataLoader(train_dataset, batch_size, shuffle=True, num_workers=num_workers)
            valid_loader = DataLoader(valid_dataset, val_batch_size, shuffle=False, num_workers=num_workers)
        else:
            train_loader = DataLoader(train_dataset, batch_size, shuffle=True, num_workers=num_workers, sampler=sampler)
