...
class Rem():
    def __init__(self,args):
        ...
        # ------------ #'--accelerator', 'ddp'
        self.args=args
        self.args.plugins = DDPStrategy(find_unused_parameters=True)
    ...
    def train(self):
        ...
        # trainer.test(model=self.model, dataloaders=iid_test_dataloader, ckpt_path=r'/root/upgrade_ckpt/old.ckpt')
        ...
        # trainer.test(model=self.model, dataloaders=ood_test_dataloader, ckpt_path=r'/root/upgrade_ckpt/old.ckpt')
...
def main_finetune():
    ...
    sys.argv += ['--num_workers', '11', '--seed', '0','--epoch' ,"1000" ,  '--batch_size', '512', '--gpus', '1',
                 '--default_root_dir', './',

                 '--ffn_dim', '2048', '--hidden_dim', '768', '--head_size', '24', '--dropout_rate', '0.1',
                 '--n_layer', '8', '--peak_lr', '2.5e-4', '--end_lr', '1e-6',
                 '--weight_decay', '0.00', '--attention_dropout_rate', '0.1', '--intput_dropout_rate', '0.1',
                 '--edge_type', 'one_hop', '--warmup_updates', '1000', '--tot_updates', '10000'
                 ]
    ...
    ###################################################################################################################
    ###################################################################################################################
    parser.add_argument('--num_workers', default=11, type=int)
