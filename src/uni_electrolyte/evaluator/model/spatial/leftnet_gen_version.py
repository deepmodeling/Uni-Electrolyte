...
class NeighborEmb(MessagePassing):
    ...
    def __init__(self, hid_dim: int, max_z: int):
        super(NeighborEmb, self).__init__(aggr='add')
        self.embedding_nb = nn.Embedding(max_z, hid_dim)
        self.hid_dim = hid_dim
        self.ln_emb = nn.LayerNorm(hid_dim,
                                   elementwise_affine=False)

    def forward(self, z, s, edge_index, embs):
        s_neighbors = self.ln_emb(self.embedding_nb(z))
...
class LEFTNet(nn.Module):
    def __init__(
            ...
            use_pbc=False, max_z=130, is_train=True
    ):
        ...
        ####################################################
        self.z_emb = Embedding(max_z, hidden_channels)
        self.n_atom_basis = hidden_channels
        self.embedding = self.z_emb
        ####################################################
        ...
        self.neighbor_emb = NeighborEmb(hidden_channels, max_z)
    ...
    def forward(self, inputs: Dict[str, torch.Tensor]):
        pos = inputs[properties.position]
        if self.is_train:
            batch = inputs[properties.idx_m]
            pos -= scatter(pos, batch, dim=0)[batch]
        # else:
        #     batch = inputs[properties.pred_idx_m]
        ...
        # inputs["vector_representation"] = s
        return inputs
